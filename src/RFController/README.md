...
This is a Python implementation of the RFOutlet tools (`codesend` and `RFSniffer`) using the `rpi-rf` library. This allows the tools to run on 64-bit Raspberry Pi OS where the original `wiringPi` library might be deprecated or unavailable.
...
you can buy more RF receivers here: https://a.co/d/1KDWkzS

you can buy more outlets here: https://www.amazon.com/dp/B00DQELHBS?ref=ppx_yo2ov_dt_b_fed_asin_title
...
### Raspberry Pi 4 GPIO Header Reference

Here is a visual guide to the Raspberry Pi GPIO header. The pins used by this project are highlighted.

```text
      +-----+-----+
 3V3  |  1  |  2  |  5V    <-- VCC (Receiver)
 SDA  |  3  |  4  |  5V    <-- VCC (Transmitter)
 SCL  |  5  |  6  |  GND   <-- GND (Transmitter)
 G04  |  7  |  8  |  TXD
 GND  |  9  | 10  |  RXD   <-- GND (Receiver)
 G17  | 11  | 12  |  G18   <-- DATA (Transmitter - GPIO 17)
 G27  | 13  | 14  |  GND   <-- DATA (Receiver - GPIO 27)
 G22  | 15  | 16  |  G23
 3V3  | 17  | 18  |  G24
 G10  | 19  | 20  |  GND
 G09  | 21  | 22  |  G25
...
**Legend:**
- `| XX |` : Physical Pin Number
- `GXX`    : BCM GPIO Number (used in code)
- `5V`     : 5 Volts Power
- `3V3`    : 3.3 Volts Power
- `GND`    : Ground
...
### Redis Event Listener (redis_listener.py)

This script subscribes to a Redis channel (`rf_commands`) and executes controller commands when it receives a JSON message.

1.  **Start Redis** (using Docker is recommended):
    ```bash
    docker-compose up -d
    ```
